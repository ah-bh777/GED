...
export default function EmployeeDirectory() {
  ...
  const [selectEntite, setSelectEntite] = useState("");
...
const filterData = () => {
  ...
  const filtered = originalData.filter((employee) => {
    ...
    // Only apply avertissements filter if a value was explicitly set (including 0)
    const matchesAvertis = theAvertis === "" 
      ? true 
      : employee.infosSupp.avertissements == theAvertis;

    const matchesConseils = theConseils === "" 
      ? true 
      : (employee.infosSupp.conseils || 0) === Number(theConseils);
    ...
    // New filters for unite and entite
    const matchesUnite = selectUnite === "" || 
      employee.infosSupp.nomUnite === selectUnite;

    const matchesEntite = selectEntite === "" || 
      employee.infosSupp.entite === selectEntite;
        return matchesSearch && matchesCorps && matchesGrade && matchesStatut && 
              matchesAvertis && matchesConseils && matchesDate && matchesDocStatus &&
              matchesUnite && matchesEntite;
      });
...
useEffect(() => {
  filterData();
}, [searchTerm, selectSearch, selectGrade, curCheckBox, theAvertis, firstDate, secondDate, docStatusFilter,
selectUnite, selectEntite, originalData, theConseils]);
  ...
  const handleConseils= (conseil) => {
    setTheConseils(conseil);
    setAdvancedFiltersUsed(true);
  };
    ...
    <div className="bg-white p-4 rounded-lg shadow mb-6">
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <button 
            ...
            <FaFilter />
            <span>Advanced</span>
            {(advancedFiltersUsed || selectUnite || selectEntite) && (
...
{(advancedFiltersUsed || selectUnite || selectEntite || theAvertis || firstDate || secondDate || docStatusFilter) && (
  <div className="w-full bg-blue-50 rounded-lg p-3 mt-2 border border-blue-100">
    <div className="flex flex-wrap gap-2 items-center">
      ...
      {selectUnite && (
        <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
          Unité: {selectUnite === "Direction Générale de la Transition Numérique" 
            ? "DG Transition Numérique" 
            : "Dépt. Réforme Admin"}
        </span>
      )}
...
<select
  value={selectUnite}
  onChange={(e) => {
    setSelectUnite(e.target.value);
    setSelectEntite("");
  }}
  className="block w-full p-2 border rounded-md"
>
  <option value="">-- Toutes les unités --</option>
  <option value="Direction Générale de la Transition Numérique">
    Direction Générale de la Transition Numérique
  </option>
  <option value="Département de la Réforme de l'Administration">
...
{selectUnite && (
  <div className="mt-3">
    ...
    <select
      value={selectEntite}
      onChange={(e) => setSelectEntite(e.target.value)}
      className="block w-full p-2 border rounded-md"
    >
      <option value="">-- Toutes les entités --</option>
      {selectUnite === "Direction Générale de la Transition Numérique" ? (
        <>
          <option value="Direction des Ecosystèmes et Entrepreneuriat Digital">
            Direction des Ecosystèmes et Entrepreneuriat Digital
          </option>
          <option value="Direction des Infrastructures Cloud et de l'Offshoring">
            Direction des Infrastructures Cloud et de l'Offshoring
          </option>
        </>
      ) : (
        <>
          <option value="Direction de la Fonction Publique">
            Direction de la Fonction Publique
          </option>
          <option value="Direction de l'Organisation de l'Administration">
            ...
            {filteredData.length > 0 ? (
              ...
              <tr>
                <td colSpan={8} className="px-6 py-4 text-center text-gray-500">
                  ...
                  theAvertis || theConseils || firstDate || secondDate || docStatusFilter || selectUnite || selectEntite
